...
#ifndef __IC3_GRAPHICS_GCI_SHARED_IMMUTABLE_STATE_CACHE_H__
#define __IC3_GRAPHICS_GCI_SHARED_IMMUTABLE_STATE_CACHE_H__
...
#define Ic3GCIStateSubCacheDefineProxy( pStateName ) \
	template <> \
	inline PipelineImmutableStateSubCache<pStateName##ImmutableState> & SharedImmutableStateCache::_SubCacheProxy<pStateName##ImmutableState>() const \
	{ \
...
namespace Ic3::Graphics::GCI
{
	...
	/**
	 * Adapter for PipelineImmutableStateFactory used by state cache. Unifies all creation methods into
	 * a single "CreateState", using overloading to automatically select the right method for a given state.
	 * The selection is based on the type of the input configuration used to create a particular state object.
	 */
	class PipelineImmutableStateFactoryCacheAdapter
	{
	public:
		PipelineImmutableStateFactoryCacheAdapter( PipelineImmutableStateFactory & pStateFactory );
		~PipelineImmutableStateFactoryCacheAdapter();
	...
	/**
	 * A "sub-cache" used by the actual cache. Manages single state type.
	 * @tparam TPState
	 */
	...
	private:
		...
		PipelineImmutableStateFactoryCacheAdapter _stateFactoryAdapter;
		/// The actual Map which serves as a storage for cached objects.
		CachedStateMap _cachedStates;
	};

	/// @brief
	class SharedImmutableStateCache
	{
	public:
		PipelineImmutableStateFactory & mStateFactory;

	public:
		SharedImmutableStateCache( PipelineImmutableStateFactory & pFactory )
...
#endif // __IC3_GRAPHICS_GCI_SHARED_IMMUTABLE_STATE_CACHE_H__
